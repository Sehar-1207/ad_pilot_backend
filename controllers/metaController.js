...
export const startMetaAuth = async (
  req,
  res
) => {
  try {
...
export const metaCallback = async (
  ...
  try {
    ...
    if (error) {
      return res.redirect(
        `${process.env.CLIENT_URL}/dashboard/settings?meta=cancelled`
      );
    }
    ...
    /*
      ...
      We need to know which Ad Pilot user
      started this OAuth flow.

      In production, use a signed OAuth
      state value tied to the logged-in user.

      Do NOT simply trust a userId coming
      from the browser.
    */
    ...
    /*
      At this point we have the Meta user
      and access token.

      We still need to associate this OAuth
      result with the logged-in Ad Pilot user.
    */

    res.redirect(
      `${process.env.CLIENT_URL}/dashboard/settings?meta=authorized&metaUserId=${metaUser.id}`
    );
  } catch (error) {
    console.error(
      "Meta callback error:",
    ...
    try {
      ...
      if (!user.metaAccessToken) {
        return res.status(400).json({
          ...
          code: "META_NOT_CONNECTED",
          message:
            "Complete Meta authorization first",
        });
      }

      /*
        Verify that this account is actually
        accessible using this user's token.
      */
      ...
      if (!account) {
        return res.status(403).json({
          success: false,
          message:
            "You do not have access to this ad account",
        });
      }
  ...
  try {
    const user =
      await User.findById(
        ...
        "metaUserId metaAdAccountId metaAdAccountName metaTokenExpiresAt isMetaConnected"
      );
...
export const syncMetaData = async (
  ...
  try {
    ...
    /*
      Next step:
      save campaigns + insights into MongoDB.
    */
